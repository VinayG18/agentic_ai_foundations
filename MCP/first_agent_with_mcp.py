...
# model = ChatNVIDIA(model="mistralai/mistral-nemo-12b-instruct", temperature=0, top_p=0.95, max_completion_tokens=4096, parallel_tool_calls=False)
model = ChatGroq(
            ...
            if msg.type == "human":
                ...
                if msg.tool_calls:
                    for tc in msg.tool_calls:
                        print(f"\U0001F680 Agent thinks -> calling: {tc['name']}({tc['args']})")
                elif msg.content:
                    print(f"\U0001F680 Agent answer: {msg.content}")
            elif msg.type == "tool":
